...
contract SupplyChain is ConsumerRole, DistributorRole, FarmerRole, RetailerRole{
  ...
  modifier harvested(uint _upc) {
    require(items[_upc].itemState == State.Harvested, "State Mismatch. Not Harvested");
    _;
  }
  ...
  string memory _originFarmLatitude, string memory _originFarmLongitude, string memory _productNotes) onlyFarmer public 
  {
    ...
    distributorID: address(uint160(0)),
    retailerID: address(uint160(0)),
    consumerID: address(uint160(0))});
    items[_upc] = item;
    // Increment sku
    sku = sku + 1;
  ...
  function buyItem(uint _upc) onlyDistributor public payable 
    ...
    address payable farmerID = address(uint160(items[_upc].ownerID));
